...
async function loadModels() {
  await faceapi.nets.faceLandmark68Net.loadFromDisk("face-api");
  await faceapi.nets.ssdMobilenetv1.loadFromDisk("face-api");
  await faceapi.nets.faceExpressionNet.loadFromDisk("face-api");
  auModel = await faceapi.tf.loadLayersModel(
    "file://./models/Krist/model.json"
  );
}

loadModels();

async function getLandmarksAUs(base64image, res) {
  ...
  const canvasInput = canvas.createCanvas(1000, 1000);
  const ctx1 = canvasInput.getContext("2d");
  await canvas.loadImage(base64image).then((image) => {
    ctx1.drawImage(image, 0, 0);
  });

  const outputCropCanvas = canvas.createCanvas(imageDims, imageDims);
  const ctx2 = outputCropCanvas.getContext("2d");

  // respond to req with status vector
  res.send(await newGenerateStatus(canvasInput, imageDims, outputCropCanvas));
}
...
async function newGenerateStatus(canvasInput, imageDims, outputCropCanvas) {
  ...
  if (typeof detections != "undefined") {
    // a face was detected
    var tempCanvasCropped = canvas.createCanvas(imageDims, imageDims);
    ...
    const resizedLandmarks = resizeLandmarks(
      detections.landmarks._positions,
      detections.detection._box,
      useSide
    );
    ...
    cropMaskImage(
      ...
      rotatedLandmarks,
      rollAngle
    ); // tempCanvasCropped now contains masked and cropped image
    copyToOutputCropCanvas(tempCanvasCropped, outputCropCanvas);
    const predictedAUs = await predictAUs(tempCanvasCropped);

    statusVector.e = Object.keys(resizedDetections.expressions).reduce((a, b) =>
      resizedDetections.expressions[a] > resizedDetections.expressions[b]
        ? a
        : b
    );
  ...
  debugging && showProcessingTime
    ? console.log("Time passed: " + statusVector.pt + "ms")
    : "";
  debugging && showStatusVector
...
async function predictAUs(canvas) {
  var inputImage = faceapi.tf.browser
    .fromPixels(canvas)
    .mean(2)
    .toFloat()
    .expandDims(0)
...
function cropRotateFace(
  ...
  const tempCanvas1 = canvas.createCanvas(imageDims, imageDims);
...
function maskFaceNew(faceLandmarks, tempCanvasCropped) {
  ...
  const eyeEyeBrowDistance = Math.sqrt(
    Math.pow(faceLandmarks[37][0] - faceLandmarks[19][0], 2) +
      Math.pow(faceLandmarks[37][1] - faceLandmarks[19][1], 2)
  );
